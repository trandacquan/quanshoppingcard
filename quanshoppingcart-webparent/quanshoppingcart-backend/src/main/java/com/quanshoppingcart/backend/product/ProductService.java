...
import com.quanshoppingcart.common.exception.ProductNotFoundException;
...
public class ProductService {
	...
	public void saveProductPrice(Product productInForm) {
		...
		productInDB.setCost(productInForm.getPrice());
		productInDB.setDiscountPercent(productInForm.getDiscountPercent());
	}

	/* Hàm kiểm tra có bị trùng */
	...
	/* Hàm xóa đối tượng product theo id */
	...
	/* Hàm lấy đối tượng product theo id */
	...
	public Page<Product> searchProducts(int pageNum, String sortField, String sortDir, String keyword) {
		...
		Pageable pageable = PageRequest.of(pageNum - 1, PRODUCTS_PER_PAGE, sort);
		return repo.searchProductsByName(keyword, pageable);// @Query("SELECT p FROM Product p WHERE p.name LIKE %?1%")
															// public Page<Product> searchProductsByName(String keyword,
															// Pageable pageable);
	}
